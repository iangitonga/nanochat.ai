...
public:
    ...
    ZephyrTokenizer(const std::string vocab_path, const int n_vocab)
        : n_vocab_{n_vocab}
    {
    ...
    ZephyrTokenizer& operator=(gten::ZephyrTokenizer &&rhs)
    {
        if (this != &rhs) {
    ...
    const char* decode(int /*to look like sentencepiece*/, const int32_t token_id) { return id_to_token_[token_id].c_str(); }
